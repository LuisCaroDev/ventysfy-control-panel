...
export const fallback: RequestHandler = async ({ request, params, url, locals, cookies }) => {
  // Invoicing API also needs authorization, though it's the same token
  if (!locals.accessToken) {
    error(401, 'Unauthorized');
  }

  const { INVOICING_URL } = getDomains(cookies.get('api_env'));
  const path = params.path;
  const queryString = url.searchParams.toString();
  const targetUrl = `${INVOICING_URL}/${path}${queryString ? '?' + queryString : ''}`;
  ...
  try {
    ...
    console.error('Invoicing proxy error:', err);
    error(500, 'Internal Server Error while proxying invoicing request');
  }
};
